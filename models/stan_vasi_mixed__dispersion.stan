...
transformed parameters {
  ...
  for (i in 1:N) {
    mu[i]  = inv_logit( X[i,] * beta + bib[id[i]]);   
    //psi[i] = inv_logit(Y[i,] * gamma); //+ Y[i,] * gamma
    psi[i] =Phi(dot_product(Y[i], gamma) +  bid[id[i]]);
...
model {
  ...
  for(j in 1:M){
  bib[j] ~ normal(0,sigma2b);
  bid[j] ~ normal(0,sigma2d);
  }
...
generated quantities{
  ...
  real EAIC= -2*likel+2*(r+k+2+25+25);
  real EBIC= -2*likel+log(N)*(r+k+2+25+25);
