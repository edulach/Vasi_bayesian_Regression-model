...
Y <- rep(1,N)
y <-df[,3]
M <- length(unique(df$departamento))
M
id <- df$departamento
id

dat_stan <- list(N = N, X = as.matrix(X),Y = as.matrix(Y), M=M,id=id,r = 2, k = 1, y = y)
#4. Prepare Data for STAN-location model
rstan_options(auto_write = TRUE)
options(mc.cores = parallel::detectCores())
# Define model files
model_files <- list(
  vasi = "models/stan_vasi_mixed_loc.stan"#stan_vasi_mixed_loc #stan_vasi_mixed_loc_dis
